    ...
    const double                                        scalar) const
{
  ...
  std::vector<std::complex<double>> cellNonLocalHamTimesWaveMatrix(
    d_numberNodesPerElement * numberWaveFunctions, 0.0);
      ...
      for (unsigned int iElemComp = 0;
           ...
           ++iElemComp)
        {
          ...
          for (unsigned int iNode = 0; iNode < d_numberNodesPerElement; ++iNode)
            {
              dealii::types::global_dof_index localNodeId =
                d_flattenedArrayCellLocalProcIndexIdMap[elementId][iNode];
              zaxpy_(
                &numberWaveFunctions,
                &alpha1,
                                   ...
                                   const double                    scalar) const
{
  ...
  std::vector<double> cellNonLocalHamTimesWaveMatrix(d_numberNodesPerElement *
                                                       numberWaveFunctions,
                                                     0.0);
      ...
      for (unsigned int iElemComp = 0;
           ...
           ++iElemComp)
        {
          dgemm_(
            ...
            &projectorKetTimesVector[atomId][0],
            &numberWaveFunctions,
            &dftPtr->d_nonLocalProjectorElementMatricesTranspose[atomId]
                                                                [iElemComp][0],
            &numberPseudoWaveFunctions,
            &beta1,
            &cellNonLocalHamTimesWaveMatrix[0],
