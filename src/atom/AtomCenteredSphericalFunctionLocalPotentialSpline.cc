...
namespace dftfe
{
  ...
  AtomCenteredSphericalFunctionLocalPotentialSpline::AtomCenteredSphericalFunctionLocalPotentialSpline(
std::string filename,  double atomAttribute, double truncationTol, double maxAllowedTail)
  {
    d_lQuantumNumber = 0;
    ...
    if (fileReadFlag)
      {
        ...
        d_rMin = xData[0];
            ...
            const double     slopeL =
              (radialFunctionData[1][1] -
               radialFunctionData[0][1]) /
  ...
  double
  AtomCenteredSphericalFunctionLocalPotentialSpline::getRadialValue(double r) const
  {
    if (r >= d_cutOff)
      return 0.0;
  ...
  std::vector<double>
  AtomCenteredSphericalFunctionLocalPotentialSpline::getDerivativeValue(double r) const
  {
  ...
  double
  AtomCenteredSphericalFunctionLocalPotentialSpline::getrMinVal() const
  {
    return d_rMin;
  }
