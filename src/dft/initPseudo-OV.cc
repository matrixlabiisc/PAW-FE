...
namespace dftfe
{
    ...
    computeElementalOVProjectorKets()
  {
    ...
    std::vector<double> nonLocalProjectorBasisReal(maxkPoints *
                                                     numberQuadraturePoints,
                                                   0.0);
    std::vector<double> nonLocalProjectorBasisImag(maxkPoints *
                                                     numberQuadraturePoints,
                                                   0.0);

    std::vector<dataTypes::number> ZetalmDeltaVl(maxkPoints *
                                                   numberQuadraturePoints,
    ...
    const unsigned int numNonLocalAtomsCurrentProcess =
      d_nonLocalAtomIdsInCurrentProcess.size();

    d_nonTrivialPseudoWfcsPerCellZetaDeltaVQuads.clear();
    d_nonTrivialPseudoWfcsPerCellZetaDeltaVQuads.resize(numCells, 0);

    d_nonTrivialPseudoWfcsCellStartIndexZetaDeltaVQuads.clear();
    d_nonTrivialPseudoWfcsCellStartIndexZetaDeltaVQuads.resize(numCells, 0);

    d_atomIdToNonTrivialPseudoWfcsCellStartIndexZetaDeltaVQuads.clear();
    ...
    for (int iAtom = 0; iAtom < numNonLocalAtomsCurrentProcess; ++iAtom)
      {
        const int nonLocalGlobalAtomId =
          d_nonLocalAtomIdsInCurrentProcess[iAtom];

        const unsigned int numberPseudoWaveFunctions =
          d_numberPseudoAtomicWaveFunctions[nonLocalGlobalAtomId];

        int numberElementsInAtomCompactSupport =
          d_elementOneFieldIteratorsInAtomCompactSupport[nonLocalGlobalAtomId]
            .size();
        ...
        globalNonLocalAtomIdToNonTrivialPseudoWfcsCellStartIndexZetaDeltaVQuads
          [nonLocalGlobalAtomId] = std::vector<unsigned int>(numCells, 0);

        for (int iElemComp = 0; iElemComp < numberElementsInAtomCompactSupport;
             ++iElemComp)
          {
            const unsigned int elementId =
              d_elementIdsInAtomCompactSupport[nonLocalGlobalAtomId][iElemComp];
            ...
            d_atomIdToNonTrivialPseudoWfcsCellStartIndexZetaDeltaVQuads
              [iAtom][elementId] = accumTemp[elementId];
            globalNonLocalAtomIdToNonTrivialPseudoWfcsCellStartIndexZetaDeltaVQuads
    ...
    d_sumNonTrivialPseudoWfcsOverAllCellsZetaDeltaVQuads =
      std::accumulate(d_nonTrivialPseudoWfcsPerCellZetaDeltaVQuads.begin(),
                      d_nonTrivialPseudoWfcsPerCellZetaDeltaVQuads.end(),
                      0);

    unsigned int accumNonTrivialPseudoWfcsCells = 0;
    for (iElem = 0; iElem < numCells; ++iElem)
      {
        d_nonTrivialPseudoWfcsCellStartIndexZetaDeltaVQuads[iElem] =
          accumNonTrivialPseudoWfcsCells;
        accumNonTrivialPseudoWfcsCells +=
          d_nonTrivialPseudoWfcsPerCellZetaDeltaVQuads[iElem];
      }


    d_nonLocalPSP_ZetalmDeltaVl.resize(
      maxkPoints * d_sumNonTrivialPseudoWfcsOverAllCellsZetaDeltaVQuads *
        numberQuadraturePoints,
      dataTypes::number(0));
    d_nonLocalPSP_zetalmDeltaVlProductDistImageAtoms.resize(
      maxkPoints * d_sumNonTrivialPseudoWfcsOverAllCellsZetaDeltaVQuads *
    ...
    d_projectorKetTimesVectorLocalIds.resize(
      d_nonLocalAtomIdsInCurrentProcess.size());


    for (unsigned int iAtom = 0; iAtom < numNonLocalAtomsCurrentProcess;
         ++iAtom)
      {
        const unsigned int atomId = d_nonLocalAtomIdsInCurrentProcess[iAtom];
    ...
    d_nonTrivialAllCellsPseudoWfcIdToElemIdMap.clear();
        ...
        for (unsigned int iatom = 0; iatom < numNonLocalAtomsCurrentProcess;
             ++iatom)
          {
            ...
            for (unsigned int i = 0;
                 i <
                 d_cellIdToNonlocalAtomIdsLocalCompactSupportMap[ielem].size();
                 i++)
              if (d_cellIdToNonlocalAtomIdsLocalCompactSupportMap[ielem][i] ==
                  iatom)
                {
                  isNonTrivial = true;
                  break;
            ...
            if (isNonTrivial)
              {
                ...
                const unsigned int numberSingleAtomPseudoWaveFunctions =
                  d_numberPseudoAtomicWaveFunctions[globalAtomId];
                for (unsigned int ipsp = 0;
                     ipsp < numberSingleAtomPseudoWaveFunctions;
                     ++ipsp)
                  {
                    d_nonTrivialAllCellsPseudoWfcIdToElemIdMap.push_back(ielem);
                    d_projecterKetTimesFlattenedVectorLocalIds.push_back(
                      d_projectorKetTimesVectorLocalIds[iatom][ipsp]);
                  }
              }
          }
      }
    ...
    for (int iAtom = 0; iAtom < numberNonLocalAtoms; ++iAtom)
      {
        ...
        dealii::Point<3> nuclearCoordinates(
          atomLocations[globalChargeIdNonLocalAtom][2],
          atomLocations[globalChargeIdNonLocalAtom][3],
          atomLocations[globalChargeIdNonLocalAtom][4]);

        std::vector<int> &imageIdsList =
          d_globalChargeIdToImageIdMapTrunc[globalChargeIdNonLocalAtom];

        //
        // get the number of elements in the compact support of the current
        // nonlocal atom
        //
        int numberElementsInAtomCompactSupport =
          d_elementOneFieldIteratorsInAtomCompactSupport[iAtom].size();


        // pcout<<"Number of elements in compact support of nonlocal atom
        // "<<iAtom<<" is "<<numberElementsInAtomCompactSupport<<std::endl;
        ...
        for (int iElemComp = 0; iElemComp < numberElementsInAtomCompactSupport;
          ...
          {
            const unsigned int elementId =
              d_elementIdsInAtomCompactSupport[iAtom][iElemComp];

            cell =
              d_elementOneFieldIteratorsInAtomCompactSupport[iAtom][iElemComp];
            ...
            for (int iPseudoWave = 0; iPseudoWave < numberPseudoWaveFunctions;
                 ++iPseudoWave)
              {
                ...
                const int lQuantumNumber =
                  d_pseudoWaveFunctionIdToFunctionIdDetails[waveFunctionId][1];
                const int mQuantumNumber =
                  d_pseudoWaveFunctionIdToFunctionIdDetails[waveFunctionId][2];
                ...
                std::fill(nonLocalProjectorBasisReal.begin(),
                          nonLocalProjectorBasisReal.end(),
                          0.0);
                std::fill(nonLocalProjectorBasisImag.begin(),
                          nonLocalProjectorBasisImag.end(),
                          0.0);

                std::fill(ZetalmDeltaVl.begin(),
                          ZetalmDeltaVl.end(),
                          dataTypes::number(0.0));
                ...
                for (int iImageAtomCount = 0;
                     ...
                     ++iImageAtomCount)
                  {
                    ...
                    // const Point & chargePoint = chargeId <
                    // numberGlobalCharges?
                    ...
                    double sphericalHarmonicVal, radialProjVal,
                        ...
                        if ((x[0] * x[0] + x[1] * x[1] + x[2] * x[2]) <=
                            d_outerMostPointPseudoProjectorData
                              ...
                              d_outerMostPointPseudoProjectorData
                                [globalWaveSplineId])
                          {
                            pseudoUtils::convertCartesianToSpherical(x,
                                                                     r,
                                                                     theta,
                                                                     phi);
                            ...
                            pseudoUtils::getSphericalHarmonicVal(
                              theta,
                              phi,
                              lQuantumNumber,
                              mQuantumNumber,
                            ...
                            for (int kPoint = 0; kPoint < maxkPoints; ++kPoint)
                              {
                                angle = d_kPointCoordinates[3 * kPoint + 0] *
                                        ...
                                        d_kPointCoordinates[3 * kPoint + 2] *
                                          pointMinusLatticeVector[2];
                                nonLocalProjectorBasisReal
                                  [kPoint * numberQuadraturePoints +
                                   iQuadPoint] +=
                                  cos(angle) * projectorFunctionValue;
                                nonLocalProjectorBasisImag
                                  [kPoint * numberQuadraturePoints +
                                   iQuadPoint] +=
                                  -sin(angle) * projectorFunctionValue;

                                const double tempReal = std::cos(-angle);
                                const double tempImag = std::sin(-angle);
                                ZetalmDeltaVl[kPoint * numberQuadraturePoints +
                                              iQuadPoint] +=
                                  dataTypes::number(tempReal *
                                                      projectorFunctionValue,
                                                    tempImag *
                                                      projectorFunctionValue);
                            ...
                            for (unsigned int iDim = 0; iDim < 3; ++iDim)
                              zetalmDeltaVlProductDistImageAtoms[iQuadPoint *
                                                                   3 +
                                                                 iDim] +=
                    ...
                    for (int iQuadPoint = 0;
                      ...
                      d_nonLocalPSP_ZetalmDeltaVl
                        [kPoint *
                         ...
                         (startIndex2 + iPseudoWave) * numberQuadraturePoints +
                         iQuadPoint] =
                          ZetalmDeltaVl[kPoint * numberQuadraturePoints +
                                        iQuadPoint];
        ...
        for (int iElemComp = 0; iElemComp < numberElementsInAtomCompactSupport;
             ++iElemComp)
          {
            ...
            d_nonLocalProjectorElementMatricesConjugate[iAtom][iElemComp]
              .resize(maxkPoints * numberNodesPerElement *
                        numberPseudoWaveFunctions,
...
#else
            d_nonLocalProjectorElementMatricesConjugate[iAtom][iElemComp]
              .resize(numberNodesPerElement * numberPseudoWaveFunctions, 0.0);
            d_nonLocalProjectorElementMatricesTranspose[iAtom][iElemComp]
              .resize(numberNodesPerElement * numberPseudoWaveFunctions, 0.0);
    ...
    initNonLocalPseudoPotential_OV()
  {
    ...
    //
    // this is the data structure used to store splines corresponding to
    // projector information of various atom types
    //
    ...
    // (i.e. map from atomicNumber to a 2D vector storing atom specific
    // projector Id and its corresponding radial and angular Ids)
    // (atomicNumber->[projectorId][Global Spline Id, l quantum number, m
    // quantum number]
    //
    std::map<unsigned int, std::vector<std::vector<int>>>
      atomicNumberToWaveFunctionIdDetails;
    std::map<unsigned int, std::vector<std::vector<int>>>
                                                             atomicNumberToPotentialIdMap;
    ...
    for (std::set<unsigned int>::iterator it = atomTypes.begin();
         ...
         ++it)
      {
        ...
        if (d_dftParamsPtr->verbosity >= 2)
          pcout << "Reading data from file: " << pseudoAtomDataFile
                << std::endl;
        ...
        // [Atomic wavefunction id](global spline id, l quantum number, m
        // quantum number)
        //
        std::vector<std::vector<int>> atomicFunctionIdDetails;

        //
        // store the number of single-atom projectors associated with the
        ...
        std::vector<int> lquantum(numberAtomicWaveFunctions),
          mquantum(numberAtomicWaveFunctions);
            ...
            if (d_dftParamsPtr->verbosity >= 2)
              {
                pcout << "Radial and Angular Functions Ids: "
                      ...
                      << radAndAngularFunctionId[1] << " "
                      << radAndAngularFunctionId[2] << std::endl;
        ...
        //
        std::vector<alglib::spline1dinterpolant> atomicSplines(
          splineFunctionIds.size());
        std::vector<alglib::real_1d_array> atomicRadialNodes(
          splineFunctionIds.size());
        std::vector<alglib::real_1d_array> atomicRadialFunctionNodalValues(
          splineFunctionIds.size());
        std::vector<double> outerMostRadialPointProjector(
          splineFunctionIds.size());

        // pcout << "Number of radial Projector wavefunctions for atomic number
        // "
        // << atomicNumber << " is: " << radFunctionIds.size() << std::endl;
            ...
            for (int iProj = 0; iProj < numProj; ++iProj)
              {
                ...
                atomicRadialFunctionNodalValues[projId].setcontent(numRows,
                                                                   yData);

                alglib::ae_int_t natural_bound_type = 1;
                alglib::spline1dbuildcubic(
                  atomicRadialNodes[projId],
                  atomicRadialFunctionNodalValues[projId],
                  numRows,
                  natural_bound_type,
                  0.0,
        ...
        d_pseudoWaveFunctionSplines.insert(d_pseudoWaveFunctionSplines.end(),
                                           atomicSplines.begin(),
                                           atomicSplines.end());
        d_outerMostPointPseudoProjectorData.insert(
          d_outerMostPointPseudoProjectorData.end(),
          outerMostRadialPointProjector.begin(),
          outerMostRadialPointProjector.end());
        ...
        if (numberAtomicWaveFunctions > 0)
          {
            ...
            d_numberPseudoAtomicWaveFunctions.push_back(
              numberAtomicWaveFunctions);
          }


        //
        // Add the atomic wave function details to the global wave function
        // vectors
        //
        for (unsigned iAtomWave = 0; iAtomWave < numberAtomicWaveFunctions;
             ++iAtomWave)
          {
            d_pseudoWaveFunctionIdToFunctionIdDetails.push_back(
              atomicFunctionIdDetails[iAtomWave]);
          }
        ...
        for (int iPseudoWave = 0; iPseudoWave < numberPseudoWaveFunctions;
             ++iPseudoWave)
          {
            d_nonLocalPseudoPotentialConstants[iAtom][iPseudoWave] =
              denominatorData[atomLocations[iAtom][0]]
                             [projector[atomLocations[iAtom][0]][iPseudoWave]]
                             [projector[atomLocations[iAtom][0]][iPseudoWave]];
            // d_nonLocalPseudoPotentialConstants[iAtom][iPseudoWave]
            ...
            if (d_dftParamsPtr->verbosity >= 4)
              pcout
                << "The value of 1/nlpConst corresponding to atom and lCount "
  ...
  dftClass<FEOrder,FEOrderElectro, memorySpace>::computeSparseStructureNonLocalProjectors_OV()
  {
              ...
              if (cell->is_locally_owned())
                {
                  ...
                  for (int iImageAtomCount = 0;
                       ...
                       ++iImageAtomCount)
                    {
                      ...
                      for (int iPsp = 0; iPsp < numberPseudoWaveFunctions;
                           ++iPsp)
                        {
    ...
    for (int iElem = 0; iElem < numberElements; ++iElem)
      {
        for (int iAtom = 0; iAtom < d_nonLocalAtomIdsInCurrentProcess.size();
             ++iAtom)
          {
            if (d_sparsityPattern[d_nonLocalAtomIdsInCurrentProcess[iAtom]]
                                 [iElem] >= 0)
              d_nonLocalAtomIdsInElement[iElem].push_back(
                d_nonLocalAtomIdsInCurrentProcess[iAtom]);
          }
      }
    ...
    std::vector<unsigned int> nonLocalAtomIdsSizeCurrentProcess(1);
    nonLocalAtomIdsSizeCurrentProcess[0] =
      d_nonLocalAtomIdsInCurrentProcess.size();
    ...
    std::vector<std::vector<unsigned int>> nonLocalAtomIdsInAllProcess(
      n_mpi_processes);
    unsigned int count = 0;
    for (unsigned int iProc = 0; iProc < n_mpi_processes; iProc++)
      {
        for (unsigned int j = 0; j < nonLocalAtomIdsSizesAllProcess[iProc]; j++)
          {
            nonLocalAtomIdsInAllProcess[iProc].push_back(
              nonLocalAtomIdsAllProcessFlattened[count]);
            count++;
          }
      }
    ...
    nonLocalGhostAtomIdsInCurrentProcess.subtract_set(
      nonLocalOwnedAtomIdsInCurrentProcess);

    std::vector<unsigned int> ownedNonLocalAtomIdsSizeCurrentProcess(1);
    ownedNonLocalAtomIdsSizeCurrentProcess[0] =
      nonLocalOwnedAtomIdsInCurrentProcess.n_elements();
    ...
    if (this_mpi_process == 0 && false)
      {
        for (std::map<int, int>::const_iterator it =
             ...
             it != oldToNewNonLocalAtomIds.end();
             it++)
    ...
    if (false)
      {
        ...
        nonLocalGhostAtomIdsInCurrentProcess.print(ss2);
        std::string s1(ss1.str());
        s1.pop_back();
        std::string s2(ss2.str());
        s2.pop_back();
        std::cout << "procId: " << this_mpi_process << " old owned: " << s1
                  << " old ghost: " << s2 << std::endl;
        std::stringstream ss3;
        nonLocalOwnedAtomIdsInCurrentProcessRenum.print(ss3);
        std::stringstream ss4;
        nonLocalGhostAtomIdsInCurrentProcessRenum.print(ss4);
        std::string s3(ss3.str());
        s3.pop_back();
        std::string s4(ss4.str());
        s4.pop_back();
        std::cout << "procId: " << this_mpi_process << " new owned: " << s3
    ...
    for (dealii::IndexSet::ElementIterator it =
         ...
         it++)
      {
        ...
        numberLocallyOwnedProjectors +=
          d_numberPseudoAtomicWaveFunctions[newToOldNonLocalAtomIds[*it]];
      }
    ...
    std::vector<unsigned int> numberLocallyOwnedProjectorsCurrentProcess(1);
    numberLocallyOwnedProjectorsCurrentProcess[0] =
      numberLocallyOwnedProjectors;
    ...
    d_locallyOwnedProjectorIdsCurrentProcess.clear();
    d_locallyOwnedProjectorIdsCurrentProcess.set_size(
      std::accumulate(numberLocallyOwnedProjectorsAllProcess.begin(),
    ...
    d_ghostProjectorIdsCurrentProcess.clear();
    d_ghostProjectorIdsCurrentProcess.set_size(
      std::accumulate(numberLocallyOwnedProjectorsAllProcess.begin(),
                      numberLocallyOwnedProjectorsAllProcess.end(),
                      0));
    ...
    for (dealii::IndexSet::ElementIterator it =
         ...
         it++)
      {
        std::vector<unsigned int> g(
          ghostAtomIdNumberPseudoWaveFunctions[localGhostCount]);
    ...
    if (false)
      {
        ...
        std::stringstream ss2;
        d_ghostProjectorIdsCurrentProcess.print(ss2);
        std::string s1(ss1.str());
        s1.pop_back();
        std::string s2(ss2.str());
        s2.pop_back();
    ...
    for (dealii::IndexSet::ElementIterator it =
         ...
         it++)
      {
        const int numberPseudoWaveFunctions =
          d_numberPseudoAtomicWaveFunctions[*it];

        for (unsigned int i = 0; i < numberPseudoWaveFunctions; ++i)
          {
            d_projectorIdsNumberingMapCurrentProcess[std::make_pair(*it, i)] =
              coarseNodeIdsAllProcess[oldToNewNonLocalAtomIds[*it]] + i;
          }
      }
    ...
    if (false)
      {
        for (std::map<std::pair<unsigned int, unsigned int>,
               ...
               d_projectorIdsNumberingMapCurrentProcess.begin();
             it != d_projectorIdsNumberingMapCurrentProcess.end();
             ++it)
          {
            std::cout << "procId: " << this_mpi_process << " ["
